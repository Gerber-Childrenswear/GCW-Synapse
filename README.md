...
**GTM in Cursor:** connect MCP server `user-gtm` — see [docs/GTM_ALL_CHATS_SETUP.md](docs/GTM_ALL_CHATS_SETUP.md) for desktop + Cloud Agent + team-wide setup.
...
## Takeover Artifact Manifest

Generate SHA-256 hashes for the latest cutover, verification, confidence, and runbook artifacts:

- Command: `npm run gtm:manifest:takeover`
- Verify command: `npm run gtm:manifest:verify:takeover:strict`
- Output artifacts:
	- `docs/reports/cutover/takeover-artifact-manifest.json`
	- `docs/reports/cutover/takeover-artifact-manifest-latest.json`
	- `docs/reports/cutover/takeover-artifact-manifest-verify-latest.json`

This manifest is also generated by the scheduled/manual readiness workflow and uploaded with the rest of the cutover artifacts.

Manifest provenance metadata:

- `source`: `github_actions` or `local`
- `gitSha`: commit SHA when available
- `gitRef`: branch or ref when available
- `workflowName`: GitHub Actions workflow name when generated in CI
- `workflowRunId`: GitHub Actions run ID when generated in CI
- `workflowAttempt`: GitHub Actions run attempt when generated in CI
- `actor`: GitHub actor when generated in CI

## Takeover Release Signal

Generate one consolidated GO/HOLD approval artifact from the latest cutover, verification, confidence, runbook, and manifest verification outputs:

- Command: `npm run gtm:release-signal:takeover`
- Strict command: `npm run gtm:release-signal:takeover:strict`
- Output artifacts:
	- `docs/reports/cutover/takeover-release-signal-<timestamp>.json`
	- `docs/reports/cutover/takeover-release-signal-<timestamp>.md`
	- `docs/reports/cutover/takeover-release-signal-latest.json`
	- `docs/reports/cutover/takeover-release-signal-latest.md`

This artifact is generated by the readiness workflow after manifest verification and fails strict mode when the combined verdict is HOLD.

## Takeover Artifact Consistency

Cross-check the latest cutover, verification, confidence, runbook, manifest verification, and release signal artifacts against each other to catch drift, tampering, or stale runs that individual generators cannot detect on their own:

- Command: `npm run gtm:consistency:takeover`
- Strict command (fails on any check failure): `npm run gtm:consistency:takeover:strict`
- Output artifacts:
	- timestamped `docs/reports/cutover/takeover-artifact-consistency-<timestamp>.json`
	- timestamped `docs/reports/cutover/takeover-artifact-consistency-<timestamp>.md`
	- stable `docs/reports/cutover/takeover-artifact-consistency-latest.json`
	- stable `docs/reports/cutover/takeover-artifact-consistency-latest.md`

Checks performed:

- **Provenance consistency**: `gitSha`, `gitRef`, and `workflowRunId` recorded on the manifest verification summary must match the release signal's copy of provenance.
- **Verdict consistency**: the gate, contract verification, confidence, runbook, and manifest verification statuses on disk must match what the release signal recorded, and the release signal's overall GO/HOLD verdict must be derivable from its own recorded component statuses.
- **Timestamp sanity**: every artifact's `generated_at` must not be in the future (small clock-skew tolerance via `--future_tolerance_minutes`, default 5) and must be within a configurable max age window (`--max_age_minutes`, default 1440).
- **Source-file existence**: every path referenced in the release signal's `sourceFiles` must exist on disk.
...
This is the final step in the readiness chain, generated by the readiness workflow immediately after the release signal, and its outcome is included in strict failure evaluation and the auto-filed regression issue.
...
5. `npm run gtm:manifest:takeover`
6. `npm run gtm:manifest:verify:takeover:strict`
7. `npm run gtm:release-signal:takeover:strict`
8. `npm run gtm:consistency:takeover:strict`
9. Uploads `docs/reports/cutover/` as workflow artifacts
10. Optionally posts webhook status with `npm run gtm:notify:takeover`
11. Auto-opens or updates a GitHub issue titled `Takeover readiness regression` when strict checks fail, including manifest verification and artifact consistency failure details
