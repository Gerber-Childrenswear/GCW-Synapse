...
register(({ analytics, settings, init }) => {
  const beaconUrl = settings?.beaconUrl || "https://gcw-synapse.ncassidy.workers.dev/browser/beacon";
  ...
  analytics.subscribe("product_viewed", (event) => {
    ...
    if (!product) return;
    const mapped = mapLineItem({ variant: product, quantity: 1 }, 0);
    emit("dl_view_item", {
      currencyCode: currency,
      detail: {
        actionField: { list: event.context?.document?.location?.pathname || "", action: "detail" },
        products: [mapped]
      }
    });
  });
  ...
  analytics.subscribe("product_added_to_cart", (event) => {
    ...
    if (!line) return;
    const mapped = mapLineItem(line, 0);
    emit("dl_add_to_cart", {
      currencyCode: currency,
      add: {
        actionField: { list: event.context?.document?.location?.pathname || "" },
        products: [mapped]
      }
    });
  });
